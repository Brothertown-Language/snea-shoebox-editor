...
Sort keys are generated by `LinguisticService.generate_sort_lx()` in `src/services/linguistic_service.py`. The pipeline is:

1. **NFD normalization** — decomposes diacritics.
2. **Strip combining marks** — removes diacritic characters.
3. **Straighten quotes** — fancy/curly quotes → straight ASCII equivalents.
4. **Symbol substitution** — linguistic symbols treated as letters (see below).
5. **Lowercase** — case-insensitive sort.
6. **Strip leading punctuation** — removes leading `*`, `-`, `=`, `[`, `]`, `(`, `)`.

### Symbol Substitution Convention (step 4)

This convention MUST be followed globally whenever sort keys are generated for this corpus.

| Symbol | Unicode | Sort key | Rationale |
|--------|---------|----------|-----------|
| `∞` | U+221E | `oozzz` | Algonquian letter for a long rounded vowel. Sorts after `oo` and all `oo*` entries (ooa…ooy), before any `op*` entries. The `zzz` suffix eliminates collision risk with real `ooz*` lemmas. |
| `✔` | U+2714 | *(stripped)* | Annotation mark; removed for sort purposes. |

**Sort order example**: `… oo, ooa, oob, …, ooy, ∞, op, …`
