...
1.  **Generate Cloudflare Credentials**:
    *   **User API Token** (For CI/CD & Github Secrets):
        ...
        *   Create a token with the following permissions:
            ...
            *   `Account` | `D1` | `Edit` (If available, otherwise covered by Workers)
            *   `Account` | `Cloudflare Pages` | `Edit`
            *   `Account` | `Account Settings` | `Read`
            *   `Zone` | `Workers Routes` | `Edit`
        *   **Verification**: Your token summary should confirm access to "All accounts" for the listed Workers/Pages permissions and "All zones" for Workers Routes.
    *   **Global API Key** (Required ONLY for `bootstrap_env.py` due to Python SDK limitations):
        *   Go to your [**Cloudflare Profile > API Tokens**](https://dash.cloudflare.com/profile/api-tokens).
        *   Find **Global API Key** and click **View**.
        *   *Note: This key is used once by the bootstrap script to create the D1 database.*
...
1.  **Configure Environment Variables**:
    Create or update your `.env` file in the project root with all the keys collected in Section 1 using the `PROD_` prefix:
    ```env
    # Cloudflare (Production Bootstrap)
    PROD_CF_EMAIL="your_cloudflare_email"
    PROD_CF_API_KEY="your_global_api_key"
    PROD_CF_API_TOKEN="your_cloudflare_user_token"

    # GitHub PAT (Production Bootstrap)
    PROD_GH_TOKEN="your_github_pat"

    # GitHub OAuth (Production credentials to be uploaded)
    PROD_GITHUB_CLIENT_ID="your_production_client_id"
    PROD_GITHUB_CLIENT_SECRET="your_production_client_secret"
    ```
...
2.  **Run Bootstrap Script**:
    ...
    *Note: If custom domains do not appear in your Cloudflare dashboard after running `bootstrap_env.py`, you can run the specialized domain initialization script:*

    ```bash
    python bootstrap_domains.py
    ```
    ...
    *   Configures Custom Domains for Cloudflare Pages and Workers.
    *   Generates a `wrangler.toml` file.

## 3. Manual Finalization

After running the bootstrap script, perform these final manual steps in the Cloudflare Dashboard:

1.  **Verify Custom Domains**:
    The bootstrap script attempts to configure these automatically, but you should verify them:
    *   **Cloudflare Pages (Frontend)**:
        1.  Navigate to **Workers & Pages** > **Overview** > `snea-shoebox-editor`.
