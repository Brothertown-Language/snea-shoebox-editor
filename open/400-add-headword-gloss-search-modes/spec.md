...
**HeadwordSearchEntry**
- Columns: `id` (PK), `record_id` (FK → `records.id`, ON DELETE RESTRICT), `entry_type` (VARCHAR, CHECK 'lx' or 'va', NOT NULL), `term` (VARCHAR, NOT NULL), `normalized_term` (VARCHAR, indexed)
- Indexes: `record_id`, `entry_type`, `normalized_term`
- Relationships: `Record.headword_entries` (one-to-many, back_populates)

**GlossSearchEntry**
- Columns: `id` (PK), `record_id` (FK → `records.id`, ON DELETE RESTRICT), `term` (VARCHAR, NOT NULL), `normalized_term` (VARCHAR, indexed)
- Indexes: `record_id`, `normalized_term`
- Relationships: `Record.gloss_entries` (one-to-many, back_populates)
- Note: NO `entry_type` column — structural divergence from HeadwordSearchEntry
...
| Lexeme | `SearchEntry` on `record_id` | Match on `normalized_term` | UNCHANGED — no entry_type filter |
...
| Grouping separators | `—— Focused ——`, `—— Broad ——` (SC-12) |
...
- **Fuzzy/typ-tolerant search** — No Levenshtein, trigram, or phonetic matching. Exact normalized-term matching only.
