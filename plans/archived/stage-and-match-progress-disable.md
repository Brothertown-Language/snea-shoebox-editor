...
1. ✔️ **Add `staging_in_progress` flag** — Set to `True` immediately when Stage & Match button
   is clicked (before any service calls); clear it on completion, error, or cancel.

2. ✔️ **Render progress bar "up top"** — At the top of the main panel (before pending batches,
...
3. ✔️ **Disable sidebar controls during staging** — Pass `disabled=st.session_state.get("staging_in_progress", False)`
...
4. ✔️ **Disable main panel controls during staging** — Pass `disabled=staging_in_progress` to:
   Review/Download/Discard buttons on pending batches, the Stage & Match button itself.

5. ✔️ **Add Cancel button** — In the sidebar, when `staging_in_progress` is True, show a
...
6. ✔️ **Wire progress callback to top-level bar** — Replace the `st.progress()` inside
...
- `tests/frontend/test_upload_mdf_page.py` — updated `TestStageAndMatch` to simulate two-render flow
