...
def test_security_manager():
    ...
    try:
        ...
        st.session_state["user_teams"] = [{"slug": "proto-SNEA", "organization": {"login": "Brothertown-Language"}}]
        role = SecurityManager.get_user_role(st.session_state["user_teams"])
        ...
        if not spec_perm:
            spec_perm = Permission(
                source_id=source.id, github_org="Brothertown-Language", github_team=test_team_name, role="editor"
            )
            session.add(spec_perm)
            session.commit()

        st.session_state["user_teams"] = [{"slug": test_team_name, "organization": {"login": "Brothertown-Language"}}]
