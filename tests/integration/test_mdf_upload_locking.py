...
class TestMDFUploadLocking(unittest.TestCase):
    ...
    def setUpClass(cls):
        with get_session() as session:
            session.execute(
                text(
                    "TRUNCATE user_activity_log, record_languages, edit_history, search_entries, matchup_queue, records, sources, users CASCADE"
                )
            )
    ...
    def setUp(self):
        with get_session() as session:
            ...
            rec1 = Record(
                ...
                mdf_data=f"\\lx {self.locked_lx}\n\\ge locked",
                is_locked=True,
            )
            rec2 = Record(
                lx=self.unlocked_lx,
                source_id=self.source_id,
                mdf_data=f"\\lx {self.unlocked_lx}\n\\ge unlocked",
                is_locked=False,
            )
    ...
    def test_download_locked_conflicts(self):
        entries = [{"lx": self.locked_lx, "mdf_data": f"\\lx {self.locked_lx}\n\\ge content"}]
        UploadService.stage_entries(self.user_email, self.source_id, entries)
