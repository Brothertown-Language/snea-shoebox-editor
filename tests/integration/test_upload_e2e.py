...
class TestUploadE2E(unittest.TestCase):
    ...
    def test_upload_lifecycle_matched_and_new(self):
        ...
        existing_mdf = "\\lx dog\n\\ps n\n\\ge canine"
        rec = Record(lx="dog", hm=1, ps="n", ge="canine", source_id=self.source_id, mdf_data=existing_mdf)
        ...
        with self._patch_session():
            ...
            dog_result = UploadService.apply_single(
                queue_id=dog_entry["queue_id"], user_email=self.user_email, session_id=session_id
            )
            self.assertEqual(dog_result["record_id"], rec.id)

            # Apply 'cat' (New)
            cat_result = UploadService.apply_single(
                queue_id=cat_entry["queue_id"], user_email=self.user_email, session_id=session_id
            )
    ...
    def test_upload_lifecycle_discard(self):
        ...
        with self._patch_session():
            ...
            session_id = str(uuid.uuid4())
            result = UploadService.apply_single(queue_id=q_id, user_email=self.user_email, session_id=session_id)
