...
class TestReviewTableRendering(unittest.TestCase):
    ...
    def test_review_table_renders_entries(
        self,
        _info,
        _code,
        mock_selectbox,
        ...
        def selectbox_side_effect(*a, **kw):
            ...
            if "Filter by Status" in str(label):
                return "All Records"
            if "per page" in str(label):
                return 1
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_exact_match_defaults_to_matched(
        self, _info, _code, mock_selectbox, mock_md, mock_container, mock_columns, _btn, _subheader, mock_get_session
    ):
        ...
        self.assertEqual(
            selectbox_calls[0][1].get("index", selectbox_calls[0][0][2] if len(selectbox_calls[0][0]) > 2 else None), 0
        )
...
class TestBulkApprovalButtons(unittest.TestCase):
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_new_source_shows_approve_all_new(
        self, _info, _code, mock_selectbox, _md, mock_container, mock_columns, mock_button, _subheader, mock_get_session
    ):
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_existing_source_shows_two_bulk_buttons(
        self, _info, _code, mock_selectbox, _md, mock_container, mock_columns, mock_button, _subheader, mock_get_session
    ):
...
class TestComparisonView(unittest.TestCase):
    ...
    @patch("streamlit.info")
    def test_comparison_shows_existing_and_new(
        self,
        _info,
        mock_render,
        mock_selectbox,
        ...
        mock_selectbox.side_effect = selectbox_side_effect
        row = _make_queue_row(
            lx="ēsh", suggested_record_id=10, match_type="exact", mdf_data="\\lx ēsh\n\\ge fire (new)"
        )
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_no_match_shows_placeholder(
        self,
        mock_info,
        _code,
        mock_selectbox,
...
class TestPerRecordApplyNow(unittest.TestCase):
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_apply_now_disabled_for_ignored(
        self, _info, _code, mock_selectbox, _md, mock_container, mock_columns, mock_button, _subheader, mock_get_session
    ):
        ...
        mock_sess.query.return_value.filter_by.return_value.filter.return_value.count.return_value = 1
        mock_sess.query.return_value.filter_by.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [
            row
        ]
        mock_sess.query.return_value.first.return_value = lang
        mock_sess.get.return_value = source_obj
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_apply_now_enabled_for_create_new(
        self, _info, _code, mock_selectbox, _md, mock_container, mock_columns, mock_button, _subheader, mock_get_session
    ):
...
class TestPagination(unittest.TestCase):
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_pagination_shows_page_size_selector(
        self, _info, _code, mock_selectbox, mock_md, mock_container, mock_columns, _btn, _subheader, mock_get_session
    ):
    ...
    @patch("streamlit.code")
    @patch("streamlit.info")
    def test_pagination_only_shows_page_size_entries(
        self, _info, _code, mock_selectbox, mock_md, mock_container, mock_columns, _btn, _subheader, mock_get_session
    ):
...
class TestStageAndMatchDisable(unittest.TestCase):
    ...
    @patch(
        ...
        {"user_role": "editor", "user_email": "test@test.com", "upload_staged_file_id": "already-staged-id"},
    )
    ...
    @patch("streamlit.info")
    @patch("streamlit.button")
    def test_stage_button_disabled_after_staging(
        self,
        mock_button,
        _info,
        _subheader,
        ...
        stage_calls = [c for c in mock_button.call_args_list if len(c[0]) > 0 and c[0][0] == "Stage & Match"]
...
class TestManualMatchOverride(unittest.TestCase):
    ...
    @patch("streamlit.expander")
    def test_override_expander_renders(
        self,
        mock_expander,
        _text_input,
        _info,
        ...
        expander_calls = [c for c in mock_expander.call_args_list if len(c[0]) > 0 and "Change match" in str(c[0][0])]
    ...
    def test_search_returns_candidates(
        ...
        def selectbox_side_effect(*a, **kw):
            ...
            if "per page" in str(label):
                return 1
            if "Select record" in str(label):
                return "#42 — ēsh (hm 1) — fire"
            if "Status" in str(label):
                return "Create New Record"
            return "Create New Record"
        ...
        select_calls = [c for c in mock_selectbox.call_args_list if len(c[0]) > 0 and c[0][0] == "Select record"]
    ...
    @patch("streamlit.success")
    @patch("streamlit.rerun")
    def test_confirm_override_calls_backend(
        self,
        mock_rerun,
        mock_success,
        mock_confirm,
    ...
    def test_no_results_shows_info(
        ...
        info_calls = [
            c for c in mock_info.call_args_list if len(c[0]) > 0 and "No matching records found" in str(c[0][0])
        ]
...
class TestReviewFiltering(unittest.TestCase):
    ...
    def test_filter_exact_match(
        ...
        mock_sess.query.return_value.filter_by.return_value.filter.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [
            row1
        ]
    ...
    @patch("streamlit.selectbox")
    @patch("streamlit.rerun")
    def test_pagination_adjusts_to_filter(
        self, mock_rerun, mock_selectbox, _subheader, mock_columns, mock_container, _md, mock_sidebar, mock_get_session
    ):
        ...
        mock_sess.query.return_value.filter_by.return_value.filter.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = rows[
            :2
        ]
        _wrap_session_mock(mock_get_session, mock_sess)
    ...
    def test_page_size_change_resets_current_page(
        self, mock_set_pref, mock_get_pref, mock_rerun, mock_selectbox, mock_sidebar, mock_get_session
    ):
