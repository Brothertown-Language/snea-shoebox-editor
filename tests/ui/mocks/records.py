...
MOCK_RECORDS = [
    {
        "id": i,
        "source": ["Fielding/Mohegan", "Trumbull/Natick", "SNEA-General"][i % 3],
        "mdf": f"\\lx record {i}\n\\ps n\n\\ge gloss {i}\n\\inf inflection {i}",
    }
...
def render_diff_view(prev_mdf: str, curr_mdf: str):
    ...
    for symbol, content, status_cls in processed_diff:
        ...
        line_html_parts.append(f'<div class="diff-line {status_cls}" style="{bg_style}">{escaped_content}</div>')
...
""")
    ...
    num_pages = (
        (total_records + st.session_state.page_size - 1) // st.session_state.page_size if total_records > 0 else 1
    )
    ...
    c1, c2 = st.columns(2)
    if c1.button(
        "Prev", icon="◀", disabled=(st.session_state.current_page <= 1), use_container_width=True, key="sidebar_prev"
    ):
    ...
    st.markdown(
        f"<p style='text-align: center; margin-bottom: 0;'>Page {st.session_state.current_page} of {num_pages}</p>",
    ...
    view_help = (
        "Show all records"
        if st.session_state.get("view_selection_only", False)
        else "Filter list to show only selected items"
    )
...
if not filtered_records:
        ...
        with st.container(border=True):
            ...
            if is_editing:
                ...
                edited_mdf = st.text_area(
                    "MDF Editor", value=initial_val, height=200, key=f"edit_{record_id}", label_visibility="collapsed"
                )
                ...
                if toolbar[0].button(
                    selection_label, use_container_width=True, icon=selection_icon, key=f"add_selection_{record_id}"
                ):
                    if not in_selection:
    ...
    st.checkbox(
        "Structural Highlighting", key="structural_highlighting_toggle", value=st.session_state.structural_highlighting
    )
