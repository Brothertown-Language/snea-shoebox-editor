...
class TestRollbackService(unittest.TestCase):
    ...
    def test_revert_batch_changes_reverts_post_import_edits(self):
        ...
        self.assertEqual(self.session.query(EditHistory).filter_by(session_id=later_session_id).count(), 0)
    ...
    def test_revert_batch_changes_skips_locked_records(self):
        ...
        rec = Record(
            ...
            mdf_data=post_import_mdf,
            is_locked=True,
            locked_by=self.user_email,
            lock_note="Protected",
        )
        self.session.add(rec)
        self.session.flush()
    ...
    def test_delete_post_import_new_records_no_new_records(self):
        ...
        session_id = str(uuid.uuid4())
        import_ts = datetime(2025, 1, 1, 12, 0, 0, tzinfo=UTC)
    ...
    def test_delete_post_import_new_records_deletes_new_records(self):
        ...
        session_id = str(uuid.uuid4())
        import_ts = datetime(2025, 1, 1, 12, 0, 0, tzinfo=UTC)
        after_ts = datetime(2025, 1, 1, 13, 0, 0, tzinfo=UTC)
