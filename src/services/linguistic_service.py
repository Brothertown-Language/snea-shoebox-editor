...
_TSQUERY_UNSAFE = re.compile(r"[^a-zA-Z0-9]+")
...
class LinguisticService:
    ...
    def get_sources_with_counts() -> list[dict[str, Any]]:
        ...
        with get_session() as session:
            ...
            record_counts = (
                session.query(Record.source_id, func.count(Record.id).label("record_count"))
            ...
            matchup_counts = (
                session.query(MatchupQueue.source_id, func.count(MatchupQueue.id).label("matchup_count"))
    ...
    def reassign_records(from_source_id: int, to_source_id: int) -> int:
        ...
        with get_session() as session:
            ...
            try:
                ...
                logger.info(
                    f"Reassigned {rows_updated} records and {matchup_updated} matchup entries (Total: {total_updated}) from source {from_source_id} to {to_source_id}."
                )
    ...
    def get_record(record_id: int) -> dict[str, Any] | None:
        ...
        """
        with get_session() as session:
            record = session.query(Record).filter(Record.id == record_id).filter(Record.is_deleted == False).first()
            if not record:
                return None

            # Get languages
            languages = [{"id": lang.id, "code": lang.code, "name": lang.name} for lang in record.language]
    ...
    def search_records(
        ...
        with get_session() as session:
            ...
            primary_lang = (
                session.query(RecordLanguage.record_id, Language.name.label("lang_name"))
            ...
            if record_ids is not None:
                ...
                if language_id:
                    ...
                    query = (
                        query.join(rl_alias, Record.id == rl_alias.record_id)
    ...
    def get_all_records_for_export(
        ...
        with get_session() as session:
            ...
            query = (
                ...
                .outerjoin(Source, Record.source_id == Source.id)
                .filter(Record.is_deleted == False)
            )
    ...
    @staticmethod
    def stream_records_to_temp_file(
        source_id: int | None = None,
        search_term: str | None = None,
        search_mode: str = "Lexeme",
        record_ids: list[int] | None = None,
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as tmp:
                with get_session() as session:
                    ...
                    query = session.query(Record.mdf_data, Record.source_id, Record.sort_lx, Record.hm).filter(
    ...
    @staticmethod
    def create_record(**fields: Any) -> int | None:
        """
        Create a new record.
        """
    ...
    @staticmethod
    def lock_record(record_id: int, user_email: str, note: str | None = None) -> bool:
        """
    ...
    def unlock_record(record_id: int, user_email: str) -> bool:
        ...
        with get_session() as session:
            ...
            try:
                ...
                # Audit: user_activity_log
                activity = UserActivityLog(
                    user_email=user_email, action="record_unlock", details=f"Unlocked record {record_id}"
                )
                session.add(activity)
    ...
    def soft_delete_record(record_id: int, user_email: str) -> bool:
        ...
        return LinguisticService.update_record(
            record_id=record_id, user_email=user_email, change_summary="Soft delete", is_deleted=True
        )
    ...
    def hard_delete_record(record_id: int) -> bool:
        ...
        with get_session() as session:
            try:
                ...
                # 2. Delete search entries (all three tables before Record to avoid FK RESTRICT)
                session.query(SearchEntry).filter(SearchEntry.record_id == record_id).delete()
                session.query(HeadwordSearchEntry).filter(HeadwordSearchEntry.record_id == record_id).delete()
                session.query(GlossSearchEntry).filter(GlossSearchEntry.record_id == record_id).delete()
                ...
                session.query(MatchupQueue).filter(MatchupQueue.suggested_record_id == record_id).update(
