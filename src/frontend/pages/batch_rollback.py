...
def confirm_revert_dialog(session_data):
    ...
    st.error(
        "This will restore records to their post-import state, discarding all subsequent edits. Locked records will be skipped. This cannot be undone."
    )
...
def confirm_delete_new_dialog(session_data):
    ...
    st.error(
        "This will permanently delete records that were created as new entries after this batch was imported. Locked records will be skipped. This cannot be undone."
    )
...
def confirm_rollback_dialog(session_data):
    ...
    st.error(
        "This action will restore records to their previous state and delete newly created records. Superseded records will be skipped. This cannot be undone."
    )
...
def main():
    ...
    # ── Main Content ──────────────────────────────────────────────
    st.write("Undo entire upload sessions. Only reversible sessions are shown below.")
    ...
    for s in page_sessions:
        with st.container():
            ...
            if btn_cols[1].button(
                "↩️",
                key=f"revert_{s['session_id']}",
                help="Revert post-import changes to this batch",
    ...
    if total_pages > 1:
        ...
        with nav_cols[0]:
            if st.button(
                "Previous", icon="⬅️", disabled=st.session_state.rollback_page_idx == 0, use_container_width=True
            ):
        ...
        with nav_cols[1]:
            st.markdown(
                f"<p style='text-align: center;'>Page {st.session_state.rollback_page_idx + 1} of {total_pages}</p>",
        ...
        with nav_cols[2]:
            if st.button(
                "Next",
                icon="➡️",
                disabled=st.session_state.rollback_page_idx >= total_pages - 1,
                use_container_width=True,
            ):
                st.session_state.rollback_page_idx += 1
                st.rerun()
