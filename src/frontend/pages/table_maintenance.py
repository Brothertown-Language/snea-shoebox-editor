        ...
        with st.container():
            ...
            if source["record_count"] > 0:
                ...
                if action_cols[1].button(
                    "", icon="🔄", key=f"reassign_{source['id']}", help="Reassign records to another source"
                ):
                ...
                if action_cols[1].button("", icon="🗑️", key=f"delete_{source['id']}", help="Delete empty source"):
                    if LinguisticService.delete_source(source["id"]):
                        ...
                        handle_ui_error(
                            ...
                            f"Failed to delete source: {source['name']}",
                            logger_name="snea.pages.table_maintenance",
                        )
...
def render_edit_dialog(source):
    ...
    if col1.button("Save Changes", use_container_width=True):
        if LinguisticService.update_source(source["id"], name=new_name, description=new_desc):
            ...
            handle_ui_error(
                Exception("Update failed"), "Failed to update source.", logger_name="snea.pages.table_maintenance"
            )
...
def render_reassign_dialog(source, all_sources):
    ...
    target_source = st.selectbox("Select target source", options=other_sources, format_func=lambda s: s["name"])
    ...
    if target_source:
        ...
        if col1.button("Confirm Reassign", type="primary", use_container_width=True):
            ...
            if count > 0:
                ...
                handle_ui_error(
                    Exception("Reassignment failed"), "Reassignment failed.", logger_name="snea.pages.table_maintenance"
                )
...
def render_data_reprocessing_maintenance():
    ...
    st.warning(
        "This operation may take several minutes depending on the number of records. Do not close this tab until the process is complete."
    )
            ...
            if action_cols[0].button("", icon="↩️", key=f"restore_{record['id']}", help="Restore record"):
                if LinguisticService.restore_record(record["id"], user_email):
                    ...
                    handle_ui_error(
                        ...
                        f"Failed to restore record: {record['lx']}",
                        logger_name="snea.pages.table_maintenance",
                    )
...
def render_hard_delete_dialog(record):
    ...
    if col1.button("Yes, Delete Permanently", type="primary", use_container_width=True):
        if LinguisticService.hard_delete_record(record["id"]):
            ...
            handle_ui_error(
                Exception("Delete failed"),
                f"Failed to delete record #{record['id']}",
                logger_name="snea.pages.table_maintenance",
            )
