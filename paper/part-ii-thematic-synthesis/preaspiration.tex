...
\subsection{Cross-Linguistic Evidence for Preaspiration}

The newly acquired reference dictionaries provide independent evidence for
the preaspiration patterns observed in the SNEA sources:

\begin{itemize}
  \item \textbf{Arapaho register variation:} The Arapaho Dictionary (Cowell
    et al.\ 2012) documents a formal/careful vs.\ everyday pronunciation
    distinction where preaspiration and glottal stops are more salient in
    careful speech (e.g., \textit{Nih'oo3oo} $\rightarrow$ \textit{Nihoo3oo}).
    This suggests that Fidelia Fielding's careful diary speech may have
    preserved preaspiration that Prince-Speck captured, while everyday
    Mohegan had already lost it --- explaining the discrepancy between
    Prince-Speck's /hC/ notation and Fielding's modern orthography.

  \item \textbf{Watkins Cree dialect chain:} Watkins (1865) documents the
    l/n/th/y dialect chain across Cree, where the same PA segment surfaces
    differently depending on dialect. This provides a model for understanding
    why Edwards (fluent L2 Mahican) writes /h/ consistently while Williams
    (English L1 Narragansett) does not --- the difference may be as much about
    the recorder's phonological categories as about the language itself.

  \item \textbf{Brinton Lenape terminal -g:} Brinton (1888) describes
    terminal \textit{-g} in Lenape as ``a strong, suddenly-checked
    expiration'' --- essentially a glottal stop or preaspiration. This
    confirms that preaspiration-like phenomena were present across Eastern
    Algonquian and were not unique to SNEA.
\end{itemize}

These comparative data strengthen the conclusion that preaspiration was a
genuine phonological feature of SNEA languages, not a recording artifact.
The variation in reporting reflects recorder competence and attention, not
absence of the feature in the language.
\begin{table}[h]
\centering
