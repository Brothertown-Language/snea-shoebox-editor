...
\subsection{Sister-Language Dictionaries as Calibration Anchors}

Beyond the eight primary sources, a growing catalog of online Algonquian
dictionaries provides independent calibration anchors for verifying SNEA
phoneme mappings. These resources are organized by language family group
in the research cards at \texttt{.issues/research-cards/}.

\subsubsection{Eastern Algonquian (Closest Genetic Relatives)}

Eastern Algonquian is the closest genetic relative of the SNEA languages.
Key online dictionaries include:

\begin{itemize}
  \item \textbf{Mi'kmaq Talking Dictionary} (\url{https://mikmaqonline.org})
    --- 6,000+ words with audio from 3+ speakers each. Preserves PA cluster
    distinctions collapsed in SNEA colonial sources.
  \item \textbf{Passamaquoddy-Maliseet Dictionary} (\url{https://pmportal.org})
    --- 18,000+ words with video archive. Preserves PA vowel length
    distinctions that are ambiguous in Williams and Wood.
  \item \textbf{Western Abenaki Dictionary} (Day 1994) --- 2 volumes, 538 pp.
    Abenaki is the closest living relative of the SNEA languages and provides
    the best comparative data for resolving SNEA phonological questions.
  \item \textbf{Lenape Talking Dictionary} (\url{https://www.talk-lenape.org})
    --- 17,000+ words with audio. Lenape is transitional between SNEA and
    Central Algonquian, crucial for the Mahican analysis.
\end{itemize}

\subsubsection{Central Algonquian (Most Conservative Phonologically)}
...
\begin{itemize}
  \item \textbf{Cree dictionaries} (\url{https://dictionary.eastcree.org},
    \url{https://itwewina.altlab.app}) --- Cree preserves PA *e: and *i:
    distinction (collapsed in SNEA colonial sources) and PA cluster
    distinctions (*hp *ht *hk *\v{s}k) directly relevant to the SNEA
    preaspiration problem.
  \item \textbf{Ojibwe dictionaries} (\url{https://ojibwe.lib.umn.edu},
    \url{https://dictionary.nishnaabemwin.atlas-ling.ca}) --- Ojibwe provides
    the Y-dialect reference for PA *r $\rightarrow$ /y/ (vs. SNEA N-dialect
    /n/).
  \item \textbf{Menominee} (Bloomfield 1962) --- The most phonologically
    conservative Algonquian language. Preserves PA vowel length and *h
    preaspiration most faithfully of any living Algonquian language.
\end{itemize}

\subsubsection{Plains Algonquian (Divergent, Boundary Conditions)}

Plains Algonquian languages (Blackfoot, Cheyenne, Arapaho) are the most
divergent from PA and provide the outer bounds for PA reflex verification.
Cheyenne (Petter 1915, \url{https://cheyennelanguage.org}) and Arapaho
(Cowell et al.\ 2012) have undergone radical phonological changes that
provide a stress-test for PA reconstruction methods used in SNEA calibration.
