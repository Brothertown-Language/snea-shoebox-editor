...
\subsection{Cross-Linguistic Vowel System Comparison}

The newly acquired reference dictionaries provide independent models for
understanding the PA vowel system that underlies the SNEA colonial
transcriptions:

\begin{itemize}
  \item \textbf{Sauk 4-vowel length system:} The Sauk Dictionary (Whittaker
    2005) documents a clean 4-vowel length system: short \textit{a/e/i/o} and
    long \textit{â/ê/î/ô}. This provides a template for the underlying PA
    vowel system that the SNEA colonial sources were attempting to represent.
    The Sauk system suggests that Williams' extreme vowel ambiguity (2--4
    possible phonemes per grapheme) is not because Narragansett had an
    unusually complex vowel system, but because Williams lacked the
    orthographic tools to mark length distinctions that were phonemic.

  \item \textbf{Watkins Cree vowel notation:} Watkins (1865) describes a
    3-way vowel length distinction for Cree (short, long, overlong) using
    diacritics (\textit{a}, \textit{ā}, \textit{â}). This confirms that
    PA vowel length was a three-way contrast in some daughter languages,
    and that the SNEA colonial sources' failure to mark length is a
    systematic recording gap, not evidence that length was not phonemic.

  \item \textbf{Arapaho triple vowels:} The Arapaho Dictionary (Cowell
    et al.\ 2012) documents triple vowels (\textit{eee}, \textit{iii},
    \textit{ooo}, \textit{uuu}) forming two syllables. While Arapaho is
    the most divergent Algonquian language, its vowel system demonstrates
    that Algonquian languages can develop complex vowel length and quality
    distinctions that far exceed the five-letter Latin alphabet.
\end{itemize}

These comparative data confirm that the vowel ambiguity in SNEA colonial
sources is primarily a recording artifact, not a reflection of the
languages' actual phonological systems. The conversion workflows must
therefore reconstruct the underlying length and quality distinctions from
comparative evidence rather than reading them from the orthography.
