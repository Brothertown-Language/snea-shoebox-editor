...
- **CRITICAL MANDATE: NO AUTONOMOUS COMMIT PREP:** **NEVER** prepare commit scripts (`tmp/commit_task.sh`) or commit messages (`tmp/commit.msg`) unless the user has explicitly and recently (within the current session) instructed you to "prepare for a commit" or "create a commit script." You are strictly prohibited from running the `git commit` command yourself. The AI is ONLY permitted to create the script for user review. **Only the user is allowed to execute the commit script; AI execution of the script is strictly forbidden.** User review of the script is mandatory before it is run. **VIOLATING THIS IS A DISCIPLINARY OFFENSE.** YOU MUST WAIT FOR THE USER TO EXPLICITLY ASK FOR THESE FILES BEFORE CREATING THEM. DO NOT ASSUME THE TASK IS FINISHED AND PROACTIVELY PREPARE THEM.
