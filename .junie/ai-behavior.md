...
- **CRITICAL MANDATE: NO AUTONOMOUS COMMIT PREP:** **NEVER** prepare commit scripts (`tmp/commit_task.sh`) or commit messages (`tmp/commit.msg`, `tmp/commit_N.msg`) unless the user has explicitly and recently (within the current session) provided the specific instruction: "Prepare for a commit" or "Create a commit script." You are strictly prohibited from anticipating the need for these files or offering to create them. Completion of a task does **NOT** imply permission to prepare for a commit. You must wait for the user to initiate the commit phase. **VIOLATING THIS IS A DISCIPLINARY OFFENSE.** YOU MUST WAIT FOR THE USER TO EXPLICITLY ASK FOR THESE FILES BEFORE CREATING THEM. DO NOT ASSUME THE TASK IS FINISHED AND PROACTIVELY PREPARE THEM. ANY ATTEMPT TO CIRCUMVENT THIS BY SUGGESTING A COMMIT IS ALSO A VIOLATION.
- **HARD STOP:** If you reach the end of your implementation steps and the user has not asked for a commit, you **MUST** simply report the completion of the implementation and WAIT. Do not mention git, commits, or staging unless the user brings it up first.
