...
- **SUPREME DIRECTIVE (v8.6)**: Absolute prohibition on modifying any file without explicit, per-step approval. You MUST wait for "Go", "Proceed", or "Approved" for EACH INDIVIDUAL EDIT. Authorization NEVER carries over and is PLAN-SPECIFIC. **QUESTIONS ARE NOT AUTHORIZATION**: Requests for information or "what if" scenarios MUST NOT be treated as directives. Every action MUST follow a checklist verified by DIRECT INSPECTION of the codebase.
...
- **Database Connection Pooling (v1.0)**: Implemented a singleton engine pattern using `@st.cache_resource` in `src/database/connection.py`.
    - **Configuration**: `pool_size=0`, `max_overflow=10`, `pool_recycle=300` (5 mins), and `pool_pre_ping=True`.
    - **Rationale**: Prioritizes an **absolute zero idle footprint** to conserve limited Aiven connection slots during inactivity. Enforces a hard limit of 10 concurrent connections. The negligible performance overhead of fresh handshakes is acceptable for this low-traffic linguistic application. `pool_pre_ping` acts as a liveness check that integrates with the existing Aiven wake-up infrastructure.
...
- **Database Stability & Recovery (v2.0)**: As of 2026-02-15, implemented several improvements to local database stability and recovery:
    - **TCP Enablement Robustness**: TCP enablement now waits for the `postmaster.pid` file to be stable before attempting to stop the server for TCP reconfiguration. A `try-except` block ensures that if the TCP restart fails, the application gracefully falls back to the Unix socket URI instead of crashing.
    - **Extension Consolidation**: Centralized all PostgreSQL extension management (e.g., `vector`) into `MigrationManager._ensure_extensions`. Includes a retry loop with backoff to handle transient "shutting down" errors often encountered during recovery cycles, while still allowing permanent failures to propagate.
    - **Migration Resilience**: Prioritizes standard SQLAlchemy ORM usage for database seeding during app startup. Migrations that add columns must run before any seeding code that triggers ORM validation. Explicitly avoids "swallowing" schema mismatches by refraining from using raw SQL to bypass ORM validation during initialization.
...
- **Streamlit Execution Mandate (v8.7)**: Foreground Streamlit execution is strictly forbidden. All Streamlit processes (including mocks) must be started in the background using `nohup` with output redirected to `tmp/` and verified by polling logs. This ensures session persistence and prevents terminal locking. Any blocking `streamlit run` call is a critical violation.
...
- **Primary Language Logic**: Refined to be structurally aware. A language is only marked as primary if it appears in the headword section of an MDF record, before any branching tags like `\se` (Subentry), `\sn` (Sense), `\va` (Variant), or `\xv` (Example). This prevents subentry languages from being incorrectly promoted to record-level primary languages. (2026-02-15)
- **NFD Sorting Requirement**: Added a TODO to `LinguisticService` and updated `roadmap.md` to specify that record sorting must use NFD normalization with leading punctuation ignored. (2026-02-15)
- **Trigram Removal**: Removed all references to PostgreSQL `pg_trgm` and `gin_trgm_ops` as they are incompatible with the `pgserver` envelope. Search now relies on `ILIKE` for linguistic forms and Native FTS for global search. (2026-02-15)
- **Documentation & Guideline Purge**: Completed a project-wide audit and purge of trigram-related recommendations. Updated `docs/deployment/PRODUCTION_ISSUES.md` and `.junie/guidelines.md` to explicitly forbid `pg_trgm` and establish it as a hard architectural constraint. (2026-02-15)
- **Documentation Alignment**: Updated `docs/deployment/PRODUCTION_ISSUES.md` to remove obsolete recommendations for `pg_trgm` and clarify search scalability strategies. (2026-02-15)
- **Schema Change Monitoring**: Updated AI Guidelines to mandate checking `tmp/streamlit.log` for all schema-related changes (migrations, extensions) to ensure stability and catch race conditions during database startup/recovery. (2026-02-15)
- **Migration Versioning Standard**: Established a mandatory versioning format for database migrations: `YYYYMMDDSSSSS` (Year, Month, Day, and seconds-since-midnight). This value MUST reflect the actual time the migration was added. Incremental values are forbidden. (2026-02-15)
- **Production Detection Refinement**: Updated `is_production()` logic in `src/database/connection.py` and `src/logging_config.py`. The system now prioritizes an explicit `runtime.mode` setting in Streamlit secrets (e.g., `mode = "production"`). It falls back to the `getpass.getuser() == "appuser"` heuristic if the secret is missing or `st.secrets` is unavailable. (2026-02-15)
- **Production Readiness Audit**: Completed a deep dive into the codebase to identify issues impacting production deployment. (2026-02-15)
   - **Critical Findings**: Identified severe OOM risks in `UploadService` and `LinguisticService` due to loading full record sets (with large `mdf_data` and `embedding` fields) into memory.
   - **Performance Bottlenecks**: Found an N+1 commit pattern in audit logging during bulk operations and inefficient Python-side pagination/filtering in the upload review view.
   - **Network Security**: Identified missing timeouts in GitHub API calls which could lead to application hangs.
   - **Documentation**: All findings are documented in `docs/deployment/PRODUCTION_ISSUES.md`.
   - **Diacritic & Quote Insensitive Search**: As of 2026-02-15, Lexeme search is now diacritic-insensitive and quote-insensitive. 
       - **Normalization**: `LinguisticService.generate_sort_lx` now strips combining marks (diacritics), straightens fancy quotes/apostrophes, lowercases, and strips leading punctuation.
...
- **DISCRETE EXECUTION MANDATE**: As of 2026-02-16, complex data assembly and implementation tasks MUST be performed in discrete, verifiable steps with mandatory per-step inspection. Guidelines updated to Section I.4.
   - **Production DB Recovery Fix (v1.3)**: As of 2026-03-01, implemented a fix in `streamlit_app.py` to include "Name or service not known" in the transient error check during database initialization. Enriched per-retry logging with `logger.warning` and remote Aiven status information (`remote_state`) via `InfrastructureService`. If `POWEROFF` is detected during the first attempt, `InfrastructureService.start_service()` is automatically called to ensure the recovery cycle succeeds. Added `[dns=not_ready]` context for DNS resolution failures. Extended the recovery window by increasing `max_attempts` to 60 (approx. 10 minutes) with a 10s delay cap to accommodate Aiven's `REBUILDING` phase. Added a manual "Retry" button for use after all automated attempts have failed, and updated the UI message to inform users that startup can take up to 10 minutes.
- **Record Locking System (v1.0)**: As of 2026-03-01, implemented a comprehensive record-level locking system to prevent accidental overwrites during MDF uploads and manual edits.
    - **Architecture**: Record-level locking using direct columns (`is_locked`, `locked_by`, `locked_at`, `lock_note`) on the `records` table (no separate table).
    - **Enforcement**: Strict immutability for **all** roles (Admins and Editors) when a record is locked. Reject all updates if `is_locked` is True.
