...
- **Production DB Recovery Fix (v1.0)**: As of 2026-03-01, implemented a fix in `streamlit_app.py` to include "Name or service not known" in the transient error check during database initialization. This allows the exponential back-off retry mechanism to handle DNS resolution propagation delays for the Aiven PostgreSQL host.
