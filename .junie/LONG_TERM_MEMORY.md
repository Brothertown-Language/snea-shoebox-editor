...
- **Production DB Recovery Fix (v1.1)**: As of 2026-03-01, implemented a fix in `streamlit_app.py` to include "Name or service not known" in the transient error check during database initialization. Enriched per-retry logging with `logger.warning` and remote Aiven status information (`remote_state`) via `InfrastructureService` to assist with production diagnostics during recovery/start cycles. Added `[dns=not_ready]` context when DNS resolution fails.
